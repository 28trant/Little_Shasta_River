...
test <-mdy(one_EVN$date)

one_EVN <- one_EVN <- one_EVN %>% 
  mutate(dates_mmddyy = mdy(one_EVN$date),
         time_hhmmss = format(strptime(time, "%I:%M %p"), format="%H:%M:%S"))



     date_yyyymmdd = as_date(date_yyyymmdd),
         time_hhmmss = as.date(time))
