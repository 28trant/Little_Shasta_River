...
one_EVN <- rbindlist(lapply(list.files(here("data","data_TT", "EVN_data"), pattern="*.xle", full.names=TRUE), read_xle)) %>% 
...
new_MWMT_MWAT <- MWMT_MWAT %>%
  ...
  filter(date_time >= "2024-10-01 00:00:00" & date_time < "2025-10-02 00:00:00")
...
CWL_hourtemps <- location_new_Cowley_Riffle %>%
...
write.csv(cowley_MWAT_output, here("data","data_TT","Output_csv_files", "cowley_mwat.csv"))
...
write.csv(cowley_MWAT_output, here("data","data_TT","Output_csv_files", "cowley_mwmt.csv"))
